# copied from Git hackbright curriculum for ex 04
...
def replace_middle(input_list):
    """Replace all elements of the input list with the the values 42 and 37, in that order, except for the first two and last two elements."""
...
def custom_insert(input_list, index, value):
    """custom_insert(input_list, index, value) imitates input_list.insert(index, value)"""
    end_of_list=input_list[index:]
    del input_list[index:]
    custom_append(input_list,value)
    custom_extend(input_list, end_of_list)
    return input_list

def custom_remove(input_list, value):
    """custom_remove(input_list, value) imitates input_list.remove(value)"""
    for q in range(custom_len(input_list)):
    	if input_list[q]==value:
    		del input_list[q]
    		return input_list
...
def custom_pop(input_list):
    ...
    popped=input_list[custom_len(input_list)-1]
...
"""
    if custom_len(some_list)==custom_len(another_list):
    	for q in range(custom_len(another_list)):
    		if some_list[q] != another_list[q]:
    			return False
    	return True
    else:
		return False
